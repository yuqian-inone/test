...
void MSP_WDG_Init(WDOG_Type* WDGx);
...
void ATC_WDG_Init(void)
{
    ...
    MSP_WDG_Init(WDOG0);
    wdgConfig.timeout = 95999999;
    wdgConfig.control.updateEnable = ENABLE;
    wdgConfig.control.intEnable = DISABLE;
    wdgConfig.control.enableWDOG = ENABLE;
    wdgConfig.control.clkSource = WDG_CLOCK_APB;
    wdgConfig.control.prescaler = DISABLE;
    wdgConfig.control.winEnable = DISABLE;
    WDOG_Initialize(&wdgConfig);

}

void MSP_WDG_Init(WDOG_Type* WDOGx)
{

    /* USER CODE BEGIN MSP_WDOG0_Init 0 */

    /* USER CODE END MSP_WDOG0_Init 0 */
    /* USER CODE BEGIN MSP_WDOG0_Init 1 */

    /* USER CODE END MSP_WDOG0_Init 1 */
}
